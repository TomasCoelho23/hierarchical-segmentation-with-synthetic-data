...
def get_model(path_ckpt):
    # Loading model
    path_model = '../GenVal-main/ControlNet/models/cldm_v15_small.yaml' #path to the respective .ymal file
...
if __name__ == "__main__":
    ...
    # Load PhenoBench dataset
    dataset = PhenoBenchDataset(
        root="../PhenoBench_GenImg", #path to the Phenobench_GenImg folder
        split="train",  # or "val" if you want val data
        prompt_json="../captions_generation.json" #this is the json file that results from the clipper when used on Phenobench_GenImg
    )

    path_ckpt = None #path to checkpoint of the model obtained during training
    ...
    image_resolution = 1024 #1024 can prduce this images but the mask will not be accurate since it requires scalling
    ...
    output_base = None  # base path for the output images

    # Define paths
    path_gen = f"{output_base}/generated" 
    path_gt = f"{output_base}/ground_truth" 
    path_original = f"{output_base}/original"
    ...
    print(f"Saving generated image to: {path_gen}")
    print(f"Saving ground truth to: {path_gt}")
    print(f"Saving original image to: {path_original}")
    ...
    for i in range(len(dataset)):
        ...
        gen_name = f"{base_name}.png"
        gt_name = f"{base_name}.png"
        orig_name = f"{base_name}.png"
        ...
        cv2.imwrite(path_gen + "/" + gen_name, gen_image[..., ::-1])
        cv2.imwrite(path_gt + "/" + gt_name, gt)
        cv2.imwrite(path_original + "/" + orig_name, original_image[..., ::-1])